...
scope MIDI {
    ...
    insert_midi(GANONDORF_BATTLE, OS.TRUE, OS.TRUE, "Ganondorf Battle", ocarina, 122)
    insert_midi(CORNERIA, OS.TRUE, OS.TRUE, "Corneria", starfox, 58)
    insert_midi(KOKIRI_FOREST, OS.TRUE, OS.TRUE, "Kokiri Forest", ocarina, 153)
    insert_midi(DR_MARIO, OS.TRUE, OS.TRUE, "Fever", drm, 92)
    insert_midi(GAME_CORNER, OS.TRUE, OS.TRUE, "Game Corner", pokemongold, 119)
    insert_midi(SMASHVILLE, OS.TRUE, OS.TRUE, "Town Hall and Tom Nook's Store", acww, 284)
    insert_midi(STONECARVING_CITY, OS.TRUE, OS.TRUE, "Stonecarving City", warioshake, 264)
    insert_midi(FIRST_DESTINATION, OS.TRUE, OS.TRUE, "Final Destination (Melee)", melee, 100)
    insert_midi(COOLCOOLMOUNTAIN, OS.TRUE, OS.TRUE, "Snow Mountain", sm64, 249)
    insert_midi(GODDESSBALLAD, OS.TRUE, OS.TRUE, "Ballad of the Goddess", skyward, 12)
    insert_midi(SARIA, OS.TRUE, OS.TRUE, "Saria's Song", ocarina, 238)
    insert_midi(TOWEROFHEAVEN, OS.TRUE, OS.TRUE, "Luna Ascension EX", toh, 164)
    insert_midi(FOD, OS.TRUE, OS.TRUE, "Gourmet Race (Melee)", kirbysuperstar, 131)
    insert_midi(MUDA, OS.TRUE, OS.TRUE, "Muda Kingdom", sml, 186)
    insert_midi(MEMENTOS, OS.TRUE, OS.TRUE, "Last Surprise", persona5, 157)
    insert_midi(SPIRAL_MOUNTAIN, OS.TRUE, OS.TRUE, "Spiral Mountain (Ultimate)", banjokazooie, 254)
    insert_midi(N64, OS.TRUE, OS.TRUE, "Water Land", sm64, 306)
    insert_midi(MUTE_CITY, OS.TRUE, OS.TRUE, "Mute City (Brawl)", fzero, 190)
    insert_midi(BATTLEFIELD, OS.TRUE, OS.TRUE, "Battlefield", brawl, 21)
    insert_midi(MADMONSTER, OS.TRUE, OS.TRUE, "Mad Monster Mansion", banjokazooie, 168)
    ...
    insert_midi(MELEE_MENU, OS.TRUE, OS.TRUE, "Melee Menu", melee, 176)
    insert_midi(GREEN_GREENS, OS.TRUE, OS.TRUE, "Green Greens", kirbydreamland, 135)
    insert_midi(NORFAIR, OS.TRUE, OS.TRUE, "Brinstar Depths", metroid, 39)
    insert_midi(BOWSERBOSS, OS.TRUE, OS.TRUE, "Koopa's Theme", sm64, 155)
    insert_midi(POKEMON_STADIUM, OS.TRUE, OS.TRUE, "Trainer Battle", pokemonred, 285)
    insert_midi(BOWSERROAD, OS.TRUE, OS.TRUE, "Koopa's Road", sm64, 154)
    insert_midi(BOWSERFINAL, OS.TRUE, OS.TRUE, "Ultimate Koopa", sm64, 291)
    insert_midi(SMB3OVERWORLD, OS.TRUE, OS.TRUE, "Super Mario Bros. 3 (Melee)", smb3, 269)
    insert_midi(DELFINO, OS.TRUE, OS.TRUE, "Delfino Plaza", sunshine, 73)
    insert_midi(VS_KLUNGO, OS.TRUE, OS.TRUE, "Vs. Klungo", banjo2, 300)
    insert_midi(BIG_BLUE, OS.TRUE, OS.TRUE, "Big Blue", fzero, 25)
    insert_extra_midi(DSAMUS_VICTORY)
    insert_midi(ONETT, OS.TRUE, OS.TRUE, "Onett", earthbound, 199)
    insert_midi(ZEBES_LANDING, OS.TRUE, OS.TRUE, "Upper Brinstar", supermetroid, 295)
    insert_midi(FROSTY_VILLAGE, OS.TRUE, OS.TRUE, "Frosty Village", dkr, 112)
    insert_midi(EASTON_KINGDOM, OS.TRUE, OS.TRUE, "Easton Kingdom", sml, 84)
    insert_midi(WING_CAP, OS.TRUE, OS.TRUE, "Powerful Mario", sm64, 218)
    insert_midi(RBY_GYMLEADER, OS.TRUE, OS.TRUE, "Gym Leader Battle", pokemonred, 138)
    insert_midi(KITCHEN_ISLAND, OS.TRUE, OS.TRUE, "Wario Land", warioland, 304)
    insert_midi(GLACIAL, OS.TRUE, OS.TRUE, "River Stage", mvc2, 231)
    insert_midi(DK_RAP, OS.TRUE, OS.TRUE, "DK Rap", dk64, 75)
    insert_extra_midi(WARIO_VICTORY)
    insert_midi(MACHRIDER, OS.TRUE, OS.TRUE, "Mach Rider (Melee)", machrider, 166)
    insert_midi(POKEFLOATS, OS.TRUE, OS.TRUE, "Red & Blue Medley", pokemonred, 226)
    insert_midi(GERUDO_VALLEY, OS.TRUE, OS.TRUE, "Gerudo Valley", ocarina, 123)
    insert_midi(POP_STAR, OS.TRUE, OS.TRUE, "Pop Star", kirby64, 216)
    insert_midi(STAR_WOLF, OS.TRUE, OS.TRUE, "Star Wolf", starfox64, 259)
    insert_midi(STARRING_WARIO, OS.TRUE, OS.TRUE, "Starring Wario!", ddrmm, 261)
    insert_extra_midi(LUCAS_VICTORY)
    insert_midi(POKEMON_CHAMPION, OS.TRUE, OS.TRUE, "Champion Battle", pokemonred, 47)
    insert_midi(ANIMAL_CROSSING, OS.TRUE, OS.TRUE, "Title Theme (Wild World)", acww, 281)
    insert_midi(HYRULE_TEMPLE, OS.TRUE, OS.TRUE, "Temple Theme (Melee)", zelda2, 274)
    insert_midi(POLLYANNA, OS.TRUE, OS.TRUE, "Pollyanna (Melee)", earthboundb, 215)
    insert_midi(SAMBA_DE_COMBO, OS.TRUE, OS.TRUE, "Samba de Combo", mother3, 237)
    insert_midi(PORKY_MEDLEY, OS.TRUE, OS.TRUE, "Master Porky Medley", mother3, 175)
    insert_midi(UNFOUNDED_REVENGE, OS.TRUE, OS.TRUE, "Unfounded Revenge (Brawl)", mother3, 294)
    insert_midi(THE_DAYS_WHEN_MY_MOTHER_WAS_THERE, OS.TRUE, OS.TRUE, "The Days When My Mother Was There", persona5, 276)
    insert_midi(BRAWL, OS.TRUE, OS.TRUE, "Brawl Menu", brawl, 38)
    insert_midi(NBA_JAM, OS.TRUE, OS.TRUE, "NBA Jam Medley", nbajam, 192)
    insert_midi(KENGJR, OS.TRUE, OS.TRUE, "Call Me Jr.", mlb, 45)
    insert_midi(CLOCKTOWER, OS.TRUE, OS.TRUE, "Clock Tower", mvc2, 53)
    insert_midi(BEIN_FRIENDS, OS.TRUE, OS.TRUE, "Bein' Friends", earthboundb, 23)
    insert_midi(KK_RIDER, OS.TRUE, OS.TRUE, "Go K.K. Rider!", animal_crossing, 126)
    insert_midi(SNAKEY_CHANTEY, OS.TRUE, OS.TRUE, "Snakey Chantey", dkc2, 247)
    insert_midi(TAZMILY, OS.TRUE, OS.TRUE, "Mom's Hometown", mother3, 183)
    insert_midi(FLAT_ZONE, OS.TRUE, OS.TRUE, "Flat Zone", melee, 102)
    insert_midi(FLAT_ZONE_2, OS.TRUE, OS.TRUE, "Flat Zone II", brawl, 103)
    insert_midi(YOSHI_GOLF, OS.TRUE, OS.TRUE, "Yoshi's Island (Mario Golf)", mariogolf, 313)
    insert_midi(FINALTEMPLE, OS.TRUE, OS.TRUE, "Great Temple/Dark Link", zelda2, 133)
    insert_midi(OBSTACLE, OS.TRUE, OS.TRUE, "Obstacle Course", yoshis_island, 194)
    insert_midi(EVEN_DRIER_GUYS, OS.TRUE, OS.TRUE, "Even Drier Guys", mother3, 90)
    insert_midi(FZERO_MEDLEY, OS.TRUE, OS.TRUE, "F-Zero Medley", fzero, 115)
    insert_midi(PEACH_CASTLE, OS.TRUE, OS.TRUE, "Princess Peach's Castle (Melee)", smb, 219)
    insert_midi(BANJO_MAIN, OS.TRUE, OS.TRUE, "Main Title (Banjo-Kazooie)", banjokazooie, 171)
    insert_extra_midi(BOWSER_VICTORY)
    insert_midi(MULTIMAN, OS.TRUE, OS.TRUE, "Multi-Man Melee", melee, 187)
    insert_midi(TABUU, OS.TRUE, OS.TRUE, "Boss Battle Song 2", brawl, 34)
    insert_midi(GANGPLANK, OS.TRUE, OS.TRUE, "Gang-Plank Galleon", dkc, 120)
    insert_midi(FD_BRAWL, OS.TRUE, OS.TRUE, "Final Destination (Brawl)", brawl, 99)
    ...
    insert_midi(PAPER_MARIO_BATTLE, OS.TRUE, OS.TRUE, "Battle Fanfare", papermario, 19)
    insert_midi(KING_OF_THE_KOOPAS, OS.TRUE, OS.TRUE, "King of the Koopas", papermario, 151)
    insert_midi(MRPATCH, OS.TRUE, OS.TRUE, "Mr. Patch", banjo2, 185)
    insert_midi(SKERRIES, OS.TRUE, OS.TRUE, "Skerries", blastcorps, 244)
    insert_midi(BEWARE_THE_FORESTS_MUSHROOMS, OS.TRUE, OS.TRUE, "Beware the Forest's Mushrooms", smrpg, 24)
    insert_midi(FIGHT_AGAINST_BOWSER, OS.TRUE, OS.TRUE, "Fight Against Bowser", smrpg, 93)
    insert_midi(DKR_BOSS, OS.TRUE, OS.TRUE, "Boss Challenges", dkr, 35)
    insert_midi(CRESCENT_ISLAND, OS.TRUE, OS.TRUE, "Crescent Island", dkr, 64)
    insert_extra_midi(CONKER_VICTORY)
    insert_midi(RITH_ESSA, OS.TRUE, OS.TRUE, "Rith Essa", jetforce, 230)
    insert_midi(TARGET_TEST, OS.TRUE, OS.TRUE, "Targets!", melee, 273)
    insert_midi(VENOM, OS.TRUE, OS.TRUE, "Venom", starfox, 297)
    insert_midi(SURPRISE_ATTACK, OS.TRUE, OS.TRUE, "Surprise Attack", starfox2, 271)
    insert_midi(BK_FINALBATTLE, OS.TRUE, OS.TRUE, "Final Battle (Banjo-Kazooie)", banjokazooie, 97)
    insert_extra_midi(MEWTWO_VICTORY)
    insert_midi(OLE, OS.TRUE, OS.TRUE, "Ole!", conker, 198)
    insert_midi(WINDY, OS.TRUE, OS.TRUE, "Windy and Co.", conker, 310)
    insert_midi(STARFOX_MEDLEY, OS.TRUE, OS.TRUE, "Star Fox Medley (Melee)", starfox, 258)
    insert_midi(DATADYNE, OS.TRUE, OS.TRUE, "dataDyne Central: Defection", pd, 68)
    insert_midi(INVESTIGATION_X, OS.TRUE, OS.TRUE, "dataDyne Central: Investigation X", pd, 69)
    insert_midi(CRADLE, OS.TRUE, OS.TRUE, "Antenna Cradle", goldeneye, 4)
    insert_midi(MM_TITLE, OS.TRUE, OS.TRUE, "Opening Title (Mischief Makers)", mischiefmakers, 202)
    insert_midi(ESPERANCE, OS.TRUE, OS.TRUE, "Esperance", mischiefmakers, 89)
    insert_midi(SLOPRANO, OS.TRUE, OS.TRUE, "Sloprano", conker, 246)
    insert_extra_midi(WOLF_VICTORY)
    insert_midi(NSMB, OS.TRUE, OS.TRUE, "Overworld Theme", nsmb, 208)
    insert_midi(JUNGLEJAPES, OS.TRUE, OS.TRUE, "Jungle Japes (Melee)", dkc, 147)
    insert_midi(FOREST_INTERLUDE, OS.TRUE, OS.TRUE, "Forest Interlude", dkc2, 107)
    insert_midi(TOADS_TURNPIKE, OS.TRUE, OS.TRUE, "Toad's Turnpike", mk64, 282)
    insert_midi(GB_MEDLEY, OS.TRUE, OS.TRUE, "Game Boy Medley", ssbr, 118)
    insert_midi(BUBBLY, OS.TRUE, OS.TRUE, "Bubbly Clouds", kirbydreamland, 41)
    insert_midi(ROADTOCERULEANCITY, OS.TRUE, OS.TRUE, "Road to Cerulean City", pokemonred, 232)
    insert_midi(LEVEL1_WARIO, OS.TRUE, OS.TRUE, "Stage 1 (Wario Land)", warioland, 256)
    insert_midi(MABE, OS.TRUE, OS.TRUE, "Mabe Village", awakening, 165)
    insert_midi(REST, OS.TRUE, OS.TRUE, "Rest Area", ssbr, 228)
    insert_midi(FE_MEDLEY, OS.TRUE, OS.TRUE, "Fire Emblem Medley", fire_emblem, 101)
    insert_midi(YOSHI_TALE, OS.TRUE, OS.TRUE, "Yoshi's Tale", yoshis_story, 315)
    insert_midi(FLOWER_GARDEN, OS.TRUE, OS.TRUE, "Flower Garden", yoshis_island, 104)
    insert_midi(WILDLANDS, OS.TRUE, OS.FALSE, "Wildlands", yoshis_island_ds, 308)
    insert_midi(VS_MARX, OS.FALSE, OS.TRUE, "Vs. Marx", kirbysuperstar, 301)
    insert_extra_midi(MARTH_VICTORY)
    insert_midi(SS_AQUA, OS.TRUE, OS.TRUE, "S.S. Aqua", pokemongold, 236)
    insert_midi(METAL_BATTLE, OS.TRUE, OS.TRUE, "Metal Battle", melee, 180)
    insert_midi(SLIDER, OS.TRUE, OS.TRUE, "Slider", sm64, 245)
    insert_midi(MULTIMAN2, OS.TRUE, OS.TRUE, "Multi-Man Melee 2", melee, 188)
    insert_midi(FIRE_EMBLEM, OS.TRUE, OS.TRUE, "Together We Ride (Melee)", fire_emblem, 283)
    insert_midi(KANTO_WILD_BATTLE, OS.TRUE, OS.TRUE, "Kanto Wild Pokemon Battle", pokemongold, 149)
    insert_midi(SMB2OVERWORLD, OS.TRUE, OS.TRUE, "Super Mario Bros. 2 Overworld", smb2, 268)
    insert_midi(PIRATELAND, OS.TRUE, OS.TRUE, "Pirate Land", marioparty2, 212)
    insert_midi(TROPICALISLAND, OS.TRUE, OS.TRUE, "Yoshi's Tropical Island", marioparty, 316)
    insert_midi(FLYINGBATTERY, OS.TRUE, OS.TRUE, "Flying Battery", sonic3, 105)
    insert_midi(OPEN_YOUR_HEART, OS.TRUE, OS.TRUE, "Open Your Heart", sonicadventure, 200)
    insert_midi(SONIC2_BOSS, OS.TRUE, OS.TRUE, "Sonic 2 Boss", sonic2, 250)
    insert_extra_midi(SONIC_VICTORY)
    insert_midi(CASINO_NIGHT, OS.TRUE, OS.TRUE, "Casino Night Zone", sonic2, 46)
    insert_midi(MONKEY_WATCH, OS.TRUE, OS.TRUE, "Monkey Watch", rhfever, 184)
    insert_midi(SONIC2_SPECIAL, OS.TRUE, OS.TRUE, "Sonic 2 Special Stage", sonic2, 251)
    insert_midi(SONICCD_SPECIAL, OS.TRUE, OS.TRUE, "Sonic CD Special Stage", soniccd, 252)
    insert_midi(GIANTWING, OS.TRUE, OS.TRUE, "Giant Wing (Fire Stone)", sonicfighters, 125)
    insert_midi(EMERALDHILL, OS.TRUE, OS.TRUE, "Emerald Hill Zone", sonic2, 87)
    insert_midi(LIVE_AND_LEARN, OS.TRUE, OS.TRUE, "Live and Learn", sonicadventure2, 162)
    insert_midi(STARDUST, OS.TRUE, OS.TRUE, "Stardust Speedway B Mix", soniccd, 260)
    insert_midi(GREEN_HILL_ZONE, OS.TRUE, OS.TRUE, "Green Hill Zone", sonic1, 136)
    insert_midi(CHEMICAL_PLANT, OS.TRUE, OS.TRUE, "Chemical Plant Zone", sonic2, 49)
    insert_midi(BABY_BOWSER, OS.TRUE, OS.TRUE, "Baby Bowser", yoshis_island, 11)
    insert_midi(WIDE_UNDERWATER, OS.TRUE, OS.TRUE, "Ocean Medley", marioparty, 196)
    insert_midi(METALLIC_MADNESS, OS.TRUE, OS.TRUE, "Metallic Madness", soniccd, 181)
    insert_midi(EVERYTHING, OS.TRUE, OS.TRUE, "Everything (Super Sonic)", sonicfighters, 91)
    insert_midi(ROCKSOLID, OS.TRUE, OS.TRUE, "Rock Solid", conker, 233)
    insert_midi(RAINBOWROAD, OS.TRUE, OS.TRUE, "Rainbow Road", mk64, 224)
    insert_midi(MK64_CREDITS, OS.TRUE, OS.TRUE, "Victory Lap", mk64, 298)
    insert_midi(RACEWAYS, OS.TRUE, OS.TRUE, "Raceways", mk64, 222)
    insert_midi(LINKS_AWAKENING_MEDLEY, OS.TRUE, OS.TRUE, "Link's Awakening Medley", awakening, 159)
    insert_midi(CORRIDORS_OF_TIME, OS.TRUE, OS.TRUE, "Corridors of Time", chrono, 59)
    insert_midi(KIRBY_64_BOSS, OS.TRUE, OS.TRUE, "Kirby 64 Boss", kirby64, 152)
    insert_midi(WALUIGI_PINBALL, OS.TRUE, OS.TRUE, "Waluigi Pinball", mkds, 303)
    insert_extra_midi(MARINA_VICTORY)
    insert_extra_midi(SHEIK_VICTORY)
    insert_extra_midi(DEDEDE_VICTORY)
    insert_midi(SMB2_MEDLEY, OS.TRUE, OS.TRUE, "Super Mario Bros. 2 Medley", smb2, 267)
    insert_midi(SMW_TITLECREDITS, OS.TRUE, OS.TRUE, "Super Mario World Title/Credits", smw, 270)
    insert_midi(DRAGONKING, OS.TRUE, OS.TRUE, "Dragon King", dragonking, 82)
    insert_midi(DEDEDE, OS.TRUE, OS.TRUE, "King Dedede's Theme", kirbydreamland, 150)
    insert_midi(DRACULAS_CASTLE, OS.TRUE, OS.TRUE, "Dracula's Castle", castlevania_sotn, 80)
    insert_midi(IRON_BLUE_INTENTION, OS.TRUE, OS.TRUE, "Iron-Blue Intention", castlevania_bloodlines, 145)
    insert_midi(DRACULAS_TEARS, OS.TRUE, OS.TRUE, "Dracula's Tears", castlevania_dos, 81)
    insert_midi(WARIOWARE, OS.TRUE, OS.TRUE, "WarioWare, Inc.", warioware, 305)
    insert_midi(BLOODY_TEARS, OS.TRUE, OS.TRUE, "Bloody Tears", castlevania_2, 28)
    insert_midi(FROZEN_HILLSIDE, OS.TRUE, OS.TRUE, "Frozen Hillside", kirbyair, 113)
    insert_midi(MK_REVENGE, OS.TRUE, OS.TRUE, "Meta Knight's Revenge", kirbysuperstar, 179)
    insert_midi(SOCCER_MENU, OS.TRUE, OS.TRUE, "Main Menu (ISS64)", isoccer, 169)
    insert_midi(TROUBLE_MAKER, OS.TRUE, OS.TRUE, "Trouble Maker", mischiefmakers, 288)
    insert_midi(MAIN_MENU2, OS.TRUE, OS.TRUE, "Menu 2 (Melee)", melee, 178)
    insert_midi(WL2_PERFECT, OS.TRUE, OS.TRUE, "Perfect!", warioland2, 210)
    insert_midi(CONTROL, OS.TRUE, OS.TRUE, "Control Center", goldeneye, 57)
    insert_midi(OEDO_EDO, OS.TRUE, OS.TRUE, "Edo Castle Medley", goemon, 85)
    insert_midi(BIS_THEGRANDFINALE, OS.TRUE, OS.TRUE, "In The Final", marioluigi_bis, 143)
    insert_extra_midi(GOEMON_VICTORY)
    insert_midi(MAJORA_MIDBOSS, OS.TRUE, OS.TRUE, "Middle Boss Battle", majora, 182)
    insert_midi(WATCH_THEME, OS.TRUE, OS.TRUE, "Q Watch (Pause Menu)", goldeneye, 220)
    insert_midi(KAI_HIGHWAY, OS.TRUE, OS.TRUE, "Kai Highway", mysticalninja, 148)
    insert_midi(SMW_ATHLETIC, OS.TRUE, OS.TRUE, "Athletic Theme", smw, 9)
    insert_midi(CRATERIA_MAIN, OS.TRUE, OS.TRUE, "Crateria Surface", supermetroid, 62)
    insert_midi(SNES_RAINBOW, OS.TRUE, OS.TRUE, "Rainbow Road (SNES)", smk, 225)
    insert_midi(BRAWL_OOT, OS.TRUE, OS.TRUE, "Ocarina of Time Medley", ocarina, 195)
    insert_midi(BOSS_E, OS.TRUE, OS.TRUE, "Boss E", starfox64, 36)
    insert_midi(MARINE_FORTRESS, OS.TRUE, OS.TRUE, "Marine Fortress", waverace, 173)
    insert_midi(TWILIGHT_CITY, OS.TRUE, OS.TRUE, "Twilight City", waverace, 289)
    insert_midi(SOUTHERNISLAND, OS.TRUE, OS.TRUE, "Southern Island/Main Theme", waverace, 253)
    insert_midi(QUEST64_BATTLE, OS.TRUE, OS.TRUE, "Battle Theme (Quest 64)", quest64, 20)
    insert_midi(MUSICAL_CASTLE, OS.TRUE, OS.TRUE, "Gorgeous Musical Castle", mysticalninja, 129)
    insert_midi(DECISIVE, OS.TRUE, OS.TRUE, "Decisive", ogrebattle64, 72)
    insert_midi(BATTLEFIELDV2, OS.TRUE, OS.TRUE, "Battlefield Ver. 2", brawl, 22)
    insert_midi(BOB, OS.TRUE, OS.TRUE, "Main Theme (Super Mario 64)", sm64, 170)
    insert_midi(AREA6, OS.TRUE, OS.TRUE, "Area 6", starfox64, 5)
    insert_midi(HILLTOPCHASE, OS.TRUE, OS.TRUE, "Hilltop Chase", kirbysuperstar, 140)
    insert_midi(STATUS, OS.TRUE, OS.TRUE, "Status", mariotennis, 262)
    insert_midi(FF4BOSS, OS.TRUE, OS.TRUE, "Boss Encounter", ff4, 37)
    insert_midi(GRIMREAPERSCAVERN, OS.TRUE, OS.TRUE, "Grim Reaper's Cavern", jackbros, 137)
    insert_midi(WORLD_OF_ENVY, OS.TRUE, OS.TRUE, "Shitto Kai / World of Envy", smtif, 242)
    insert_midi(SHANTAEMEDLEY, OS.TRUE, OS.TRUE, "Day/Night Traveling", shantae, 70)
    insert_midi(BURNINGTOWN, OS.TRUE, OS.TRUE, "Burning Town", shantae, 43)
    insert_midi(SHANTAEBOSS, OS.TRUE, OS.TRUE, "Boss Battle (Shantae)", shantae, 32)
    insert_midi(FORTRESS_BOSS, OS.TRUE, OS.TRUE, "Fortress Boss", smw, 108)
    insert_midi(HORROR_LAND, OS.TRUE, OS.TRUE, "Horror Land", marioparty2, 142)
    insert_midi(AC_TITLE, OS.TRUE, OS.TRUE, "Animal Crossing Theme", animal_crossing, 3)
    insert_midi(DARKWORLD, OS.TRUE, OS.TRUE, "Dark World", lttp, 67)
    insert_midi(FILESELECT_SM64, OS.TRUE, OS.TRUE, "File Select", sm64, 95)
    insert_midi(ITSATRAP_SM64, OS.TRUE, OS.TRUE, "File Select (Alternate)", sm64, 96)
    insert_extra_midi(BANJO_VICTORY)
    insert_midi(BLASTCORPS_MENU, OS.TRUE, OS.TRUE, "Menu (Blast Corps)", blastcorps, 177)
    insert_midi(FRAPPE_SNOWLAND, OS.TRUE, OS.TRUE, "Frappe Snowland", mk64, 110)
    insert_midi(SMRPG_BATTLE, OS.TRUE, OS.TRUE, "Fight Against Monsters", smrpg, 94)
    insert_midi(TRAVELING, OS.TRUE, OS.TRUE, "Traveling", goepachisuro, 286)
    insert_midi(CHILL, OS.TRUE, OS.TRUE, "Chill", drm, 50)
    insert_midi(ROLL, OS.TRUE, OS.TRUE, "Roll's Theme", megamanbc, 234)
    insert_midi(STICKERBRUSH_SYMPHONY, OS.TRUE, OS.TRUE, "Stickerbrush Symphony", dkc2, 263)
    insert_midi(DOOM1, OS.TRUE, OS.TRUE, "DOOM Medley", doom, 78)
    insert_midi(RUNNING_FROM_EVIL, OS.TRUE, OS.TRUE, "Running From Evil", doom2, 235)
    insert_midi(CONKER_THE_KING, OS.TRUE, OS.TRUE, "Conker the King (Reprise)", conker, 56)
    insert_midi(GRABBAG, OS.TRUE, OS.TRUE, "Grabbag", dukenukem3d, 132)
    insert_midi(WITHMILASDIVINEPROTECTION, OS.TRUE, OS.TRUE, "With Mila's Divine Protection", fe_gaiden, 311)
    insert_midi(DKCTITLE, OS.TRUE, OS.TRUE, "Opening (Donkey Kong)", dkc, 201)
    insert_midi(PLANTATION, OS.TRUE, OS.TRUE, "Plantation", cavestory, 213)
    insert_midi(BATTLE_GOLD_SILVER, OS.TRUE, OS.TRUE, "Gold & Silver Medley", pokemongold, 127)
    insert_midi(BIG_BOO, OS.TRUE, OS.TRUE, "Haunted House", sm64, 139)
    insert_extra_midi(DKING_VICTORY)
    insert_midi(7AM, OS.TRUE, OS.TRUE, "7AM", animal_crossing, 1)
    insert_midi(DKROPTIONS, OS.TRUE, OS.TRUE, "Options (Diddy Kong Racing)", dkr, 203)
    insert_midi(GALLERY, OS.TRUE, OS.TRUE, "Gallery", ssbr, 117)
    insert_midi(QUEQUE, OS.TRUE, OS.TRUE, "Que Que", drm64, 221)
    insert_midi(MK64MENU, OS.TRUE, OS.TRUE, "Setup and Kart Select", mk64, 239)
    insert_midi(GOLDENROD_CITY, OS.TRUE, OS.TRUE, "Goldenrod City", pokemongold, 128)
    insert_midi(CLOCKTOWN, OS.TRUE, OS.TRUE, "Clock Town", majora, 54)
    insert_midi(BUMPERCROPBUMP, OS.TRUE, OS.TRUE, "Bumper Crop Bump", kirby64, 42)
    insert_midi(VSRIDLEY, OS.TRUE, OS.TRUE, "Vs. Ridley", supermetroid, 302)
    insert_midi(GANONMEDLEY, OS.TRUE, OS.TRUE, "Ganon Battle Medley", zelda, 121)
    insert_midi(FUGUE, OS.TRUE, OS.TRUE, "Little Fugue", ssbr, 161)
    insert_midi(NUTTY_NOON, OS.TRUE, OS.TRUE, "Nutty Noon", kirbyreturn, 193)
    insert_midi(GHOSTGULPING, OS.TRUE, OS.TRUE, "Ghost Gulping", papermario, 124)
    insert_midi(SMB2BOSS, OS.TRUE, OS.TRUE, "Super Mario Bros. 2 Boss", smb2, 266)
    insert_midi(FZERO_CLIMBUP, OS.TRUE, OS.TRUE, "Climb Up! And Get the Last Chance!", fzero_x, 52)
    insert_midi(JFG_SELECT, OS.TRUE, OS.TRUE, "Character Select (Jet Force Gemini)", jetforce, 48)
    insert_midi(YOSHI_SKA, OS.TRUE, OS.TRUE, "Yoshi's Song", yoshis_story, 314)
    insert_midi(BOARD_SHOP, OS.TRUE, OS.TRUE, "Board Shop", sbk, 30)
    insert_midi(FLANDRES_THEME, OS.TRUE, OS.TRUE, "U.N. Owen Was Her?", 2hu6, 290)
    insert_midi(NIGHTMARE, OS.TRUE, OS.TRUE, "Final Boss (Nightmare's Battle)", kirbyadventure, 98)
    insert_midi(THE_ALOOF_SOLDIER, OS.TRUE, OS.TRUE, "The Aloof Soldier", gga, 275)
    insert_midi(WENDYS_HOUSE, OS.TRUE, OS.TRUE, "Wendy's House", sbk2, 307)
    insert_midi(DANGEROUS_FOE, OS.TRUE, OS.TRUE, "Battle Against a Dangerous Foe", earthboundb, 16)
    insert_midi(BIG_SNOWMAN, OS.TRUE, OS.TRUE, "Big Snowman", sbk, 26)
    insert_midi(PIKA_CUP, OS.TRUE, OS.TRUE, "Pika Cup Battles 1-3", stadium, 211)
    insert_midi(ASHLEYS_THEME, OS.TRUE, OS.TRUE, "Ashley's Theme", wwtouched, 7)
    insert_midi(SILVER_MOUNTAIN, OS.TRUE, OS.TRUE, "Silver Mountain", sbk, 243)
    insert_midi(WIZPIG, OS.TRUE, OS.TRUE, "Wizpig Challenge", dkr, 312)
    insert_midi(TALENTSTUDIO, OS.TRUE, OS.TRUE, "Talent Studio Medley", talentstudio, 272)
    insert_midi(BATTLE_C1, OS.TRUE, OS.TRUE, "Battle C1", smtiv, 18)
    insert_midi(PLAY_A_MINIGAME, OS.TRUE, OS.TRUE, "Play a Mini-Game!", marioparty, 214)
    insert_midi(CREDITS_BRAWL, OS.TRUE, OS.TRUE, "Credits (Brawl)", ssb, 63)
    insert_extra_midi(CRASH_VICTORY)
    insert_midi(PD_PAUSE, OS.TRUE, OS.TRUE, "Pause Menu (Perfect Dark)", pd, 209)
    insert_extra_midi(PEACH_VICTORY)
    insert_midi(PORKY, OS.TRUE, OS.TRUE, "Porky Means Business!", earthbound, 217)
    insert_midi(UNDERGROUND, OS.TRUE, OS.TRUE, "Underground Theme", smb, 292)
    insert_midi(UNDERGROUND_HURRY, OS.TRUE, OS.TRUE, "Underground Theme (Hurry Up!)", smb, 293)
    insert_midi(CRASH3, OS.TRUE, OS.TRUE, "Time Twister", crash3, 279)
    insert_midi(NSANITYBEACH, OS.TRUE, OS.TRUE, "N. Sanity Beach", crash, 191)
    insert_midi(CTR_MENU, OS.TRUE, OS.TRUE, "Crash Team Racing Menu", ctr, 61)
    insert_midi(DISCOVERYFALLS, OS.TRUE, OS.TRUE, "Discovery Falls", dinoplanet, 74)
    insert_midi(CRASHBONUS, OS.TRUE, OS.TRUE, "Bonus (CB:THA)", crash_xs, 31)
    insert_midi(DK_MEDLEY, OS.TRUE, OS.TRUE, "Donkey Kong Medley", dkarc, 77)
    insert_midi(SM64STAFF, OS.TRUE, OS.TRUE, "Staff Roll (Super Mario 64)", sm64, 255)
    insert_midi(BIG_BRIDGE, OS.TRUE, OS.TRUE, "Clash on the Big Bridge", ff5, 51)
    insert_midi(HOGWILD, OS.TRUE, OS.TRUE, "Hog Wild", crash, 141)
    insert_midi(MARATHON, OS.TRUE, OS.TRUE, "Marathon 2 Title Theme", marathon2, 172)
    insert_midi(FORGONE, OS.TRUE, OS.TRUE, "Foregone Destruction", ut99, 106)
    insert_midi(MORRIGAN, OS.TRUE, OS.TRUE, "Theme of Morrigan", mvc, 278)
    insert_midi(ELADARD, OS.TRUE, OS.TRUE, "Eladard", starfox2, 86)
    insert_midi(MADMAZEMAUL, OS.TRUE, OS.TRUE, "Mad Maze Maul", dk64, 167)
    insert_midi(ORANGSPRINT, OS.TRUE, OS.TRUE, "OrangSprint", dk64, 206)
    insert_midi(DRAKE_LAKE, OS.TRUE, OS.TRUE, "Drake Lake", waverace, 83)
    insert_midi(BUBBLEGUM_KK, OS.TRUE, OS.TRUE, "Bubblegum K.K.", acnewleaf, 40)
    insert_midi(FZEROX_MEDLEY, OS.TRUE, OS.TRUE, "F-Zero X Medley", fzero_x, 116)
    insert_midi(FREEZE, OS.TRUE, OS.TRUE, "Freeze!", papermario, 111)
    insert_midi(TITANIA, OS.TRUE, OS.TRUE, "Titania", starfox, 280)
    insert_midi(THEATER, OS.TRUE, OS.TRUE, "Theater", kirby64, 277)
    insert_midi(ENEMYCARD, OS.TRUE, OS.TRUE, "Enemy Card Index", kirby64, 88)
    insert_midi(SHEVAT, OS.TRUE, OS.TRUE, "Shevat, the Wind is Calling", xenogears, 241)
    insert_midi(CORTEX, OS.TRUE, OS.TRUE, "Dr. Neo Cortex", crash2, 79)
    insert_midi(WILY_FIELD, OS.TRUE, OS.TRUE, "Wily's Field", megamanscr, 309)
    insert_midi(SMS_BOSS, OS.TRUE, OS.TRUE, "Boss Battle (Sunshine)", sunshine, 33)
    insert_midi(COBALT, OS.TRUE, OS.TRUE, "Cobalt Coast", heypika, 55)
    insert_midi(VS_DSAMUS, OS.TRUE, OS.TRUE, "Vs. Dark Samus", metroidprime3, 299)
    insert_midi(STRIKE_THE_EARTH, OS.TRUE, OS.TRUE, "Strike the Earth!", shovelknight, 265)
    insert_midi(VAMPIREKILLER, OS.TRUE, OS.TRUE, "Vampire Killer", castlevania, 296)
    insert_midi(KOOPA_BROS, OS.TRUE, OS.TRUE, "Attack of the Koopa Bros.", papermario, 10)
    insert_midi(REDIAL, OS.TRUE, OS.TRUE, "Redial", bombermanhero, 227)
    insert_midi(AGAVE, OS.TRUE, OS.TRUE, "Agave", snp, 2)
    insert_midi(RAIDBLUE, OS.TRUE, OS.TRUE, "Raid Blue", snp, 223)
    insert_midi(RISKNECK, OS.TRUE, OS.TRUE, "Risk One's Neck", snp, 229)
    insert_midi(SHERBETLAND, OS.TRUE, OS.TRUE, "Sherbet Land", mariohoops, 240)
    insert_midi(DEATH_MOUNTAIN, OS.TRUE, OS.TRUE, "Dark Mountain Forest", lttp, 66)
    insert_midi(BATTLE_AMONG_FRIENDS, OS.TRUE, OS.TRUE, "Battle Among Friends", kirby64, 17)
    insert_midi(BUTTER_BUILDING, OS.TRUE, OS.TRUE, "Butter Building", kirbyadventure, 44)
    insert_midi(MURASAKI, OS.TRUE, OS.TRUE, "Murasaki Forest", mother3, 189)
    insert_midi(SKYWORLD, OS.TRUE, OS.TRUE, "Overworld (Kid Icarus)", kidicarus, 207)
    insert_midi(FE6_MEDLEY, OS.TRUE, OS.TRUE, "Binding Blade Medley", fe6, 27)
    insert_midi(SNOWGO, OS.TRUE, OS.TRUE, "Snow Go", crash2, 248)
    insert_midi(FUTUREFRENZY, OS.TRUE, OS.TRUE, "Future Frenzy", crash3, 114)
    insert_extra_midi(LANKY_VICTORY)
    insert_midi(SILVERSURFER, OS.TRUE, OS.TRUE, "Level 1 (Silver Surfer)", silversurfer, 158)
    insert_midi(OPUS_13, OS.TRUE, OS.TRUE, "Opus 13", castlevania_rob, 205)
    insert_midi(FOURSIDE, OS.TRUE, OS.TRUE, "Fourside", earthbound, 109)
    insert_midi(BALLOONFIGHT, OS.TRUE, OS.TRUE, "Balloon Fight", balloonfight, 13)
    insert_midi(LIPS_THEME, OS.TRUE, OS.TRUE, "Lip's Theme", paneldepon, 160)
    insert_midi(CRASHBASH_LOADING, OS.TRUE, OS.TRUE, "Crash Bash Loading Screen", crashbash, 60)
    insert_midi(TREASURE_TROVE_COVE, OS.TRUE, OS.TRUE, "Treasure Trove Cove", banjokazooie, 287)
    insert_midi(STAGESELBM64, OS.TRUE, OS.TRUE, "Stage Select (Bomberman 64)", bomberman, 257)
    insert_midi(OLDKINGCOAL, OS.TRUE, OS.TRUE, "Old King Coal", banjo2, 197)
    insert_midi(SONIC_R, OS.TRUE, OS.TRUE, "Options (Sonic R)", sonicr, 204)
    insert_midi(GREENGARDEN, OS.TRUE, OS.TRUE, "Green Garden", bomberman, 134)
    insert_midi(BLUE_RESORT, OS.TRUE, OS.TRUE, "Blue Resort", bomberman, 29)
    insert_midi(LOST, OS.TRUE, OS.TRUE, "Lost", dream, 163)
    insert_midi(DK_JR_STAGE, OS.TRUE, OS.TRUE, "Donkey Kong Jr.", dkjr, 76)
    ...
    insert_midi(DCMC, OS.TRUE, OS.TRUE, "DCMC Performance", mother3, 71)
    ...
    insert_midi(LANCEBATTLE, OS.TRUE, OS.TRUE, "Lance Battle", stadium2, 156)
    insert_midi(MASKEDDEDEDE, OS.TRUE, OS.TRUE, "Masked Dedede's Theme", kirbytripledx, 174)
